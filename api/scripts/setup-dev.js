...
const promptForKey = async () => {
  ...
  return new Promise(resolve => {
    rl.question('Paste DOTENV_PRIVATE_KEY_DEV value: ', answer => {
      rl.close();
      resolve(answer.trim());
    });
  });
...
`;
  ...
  success('Private key saved to .env.keys');
};
...
const main = async () => {
  try {
    console.log('\uD83D\uDD10 dotenvx Development Setup (api)');
    ...
    console.log('To decrypt development secrets, you need the private key.');
    console.log('Ask a team member or check your secure key storage.\n');

    const privateKey = await promptForKey();
    if (!validateKey(privateKey)) process.exit(1);

    saveKey(privateKey);

    if (testDecryption()) {
      success('Development setup complete!');
      console.log('\n\uD83D\uDCDD Remember to restart: docker compose restart api\n');
    } else {
      error('Setup failed. Check your key and try again.');
      process.exit(1);
    }
  } catch (err) {

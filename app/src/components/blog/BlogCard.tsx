...
export const BlogCard = ({ article }: { article: BlogArticle }) => (
  <article>
    ...
    >
      {article.imageUrl && (
        <figure className="relative aspect-[300/160] w-full overflow-hidden">
          <Image
      ...
      <div className="flex flex-1 flex-col gap-2 p-3 md:p-4 lg:p-5">
        <div className="flex flex-wrap items-center gap-2">
          {article.categories.map(({ id, name }) => (
            <span key={id} className="rounded-full bg-gray-100 px-2 py-1 text-xs">
              {name}
            </span>
          ))}
        ...
        <h3 className="line-clamp-2 text-xl leading-[26px] font-bold lg:text-2xl lg:leading-7">{article.title}</h3>
